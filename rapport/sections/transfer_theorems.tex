...
%\section{Transfer theorem in the integer case}
%
...
%	Let $a \in \mathbb{N}$ and $b \in \mathbb{N}^*$.
%	
%	Let
%	\[
%	f : z \mapsto {(1 - z)}^{a} {\left( \frac{1}{z} \log \frac{1}{1 - z} \right)}^b
...
%	then for large $n$ one has
%	$$
%	f_n = {(-1)}^a b \Gamma(1 + a) n^{-a - 1} \psum{j = 0}^\infty \frac{F_j(\log n)}{n^k}
%	$$
%	with $deg(F_j) \leq k-1$.
%\end{thm}
%
%\begin{proof}
%	Let $$\phi_0 : z \mapsto \Gamma(a) {(1 - z)}^{-a}
